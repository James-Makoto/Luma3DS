...
clean:
	...
	@rm -rf *.firm *.zip *.3dsx luma

# boot.3dsx comes from https://github.com/fincs/new-hbmenu/releases
$(NAME)$(REVISION).zip:	hbmenu.zip boot.firm
	@cp $< $@
	@mkdir -p luma
	@cp unifont_cn.bin luma/unifont_cn.bin
	@zip $@ boot.firm luma/unifont_cn.bin -9 -x "*.DS_Store*" "*__MACOSX*"
...
hbmenu.zip:
	@curl -sSfL $(shell curl -s https://api.github.com/repos/R-YaTian/3ds-hbmenu/releases/latest | grep 'browser_' | cut -d\" -f4) -o $@
